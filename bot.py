...
bot = telebot.TeleBot("1786477323:AAFXeSvLupwQxi3gmaCUJCYpKiOzAGOYm_E")
User_bot("@Amanda_Robot") 

@bot.message_handler(commands=["frase"])
def frase_command(m):
	if m.text == '/frase' or m.text == '/frase@TioMortybot':
...
def apaixao(m):
	if m.text == '/apaixonado' or m.text == '/apaixonado@TioMortybot':
		list = ["Apaixonado (a)"]
		valor = randint(0, 100)
		resposta = choice(list)
...
def lindeza(m):
	if m.text == '/lindeza' or m.text == '/lindeza@TioMortybot':
		list = ["Lindeza (o) "]
		valor = randint(0, 100)
		resposta = choice(list)
...
def gen_markup():
    ...
    markup.add(types.InlineKeyboardButton('🌝 | Canal do Criador', url='https://telegram.me/Fraviiu')) 
...
/Start - *Iniciar o Bot *
/ajuda - *envia ajuda*
/sugerir - *solicita uma sugestão*
/calificar - *votar neste bot na store*
/frase - *Frases aleatória*
/link - *envia o link do bot*
/lindeza - *Veja o Quanto vc tem de Lindeza* 
/apaixonado - *Veja o Quanto voce esta paixonado* 
/lixomentro - *Veja o Quanto vc e de Lixo*
/novidades - *Novidades*
/contacto - *envia informações de contato*
/delicia - *Veja O Quanto Voce e Delicioso (a) *
/cheiro - *Veja o Quanto Voce e Cheirosa (o) *
/mais  -  *Veja Novos Bots *
/canal-  *canal do bot*
/dono - *Criador Do Bot*

OUTRO BOT CRIADO COM MESMA API DA AMANDA ROBOT
@JuliaRobot 
...
def delicia(m):
	if m.text == '/delicia' or m.text == '/delicia@Amanda_Robot':
		list = ["Deliciosa (o) "]
		valor = randint(0, 100)
		resposta = choice(list)
...
def cheiro(m):
	if m.text == '/cheiro' or m.text == '/cheiro@Amanda_Robot':
		list = ["Cheirosa (o) "]
		valor = randint(0, 100)
		resposta = choice(list)
		bot.reply_to(m, "{} *Você e *{}%  *de* *{}* ".format(m.from_user.first_name, valor, resposta), parse_mode='Markdown')
...
def lixo(m):
	if m.text == '/lixomentro' or m.text == '/lixomentro@Amanda_Robot':
		list = ["Lixo (a) "]
		valor = randint(0, 100)
		resposta = choice(list)
...
def sugerir(m):
	if m.text == '/sugerir' or m.text == '/sugerir@Amanda_Robot':
		list = ["Carregando..... "]
		valor = randint(0, 100)
		resposta = choice(list)
		bot.reply_to(m, "Ola {} Voce Pode Usa o Bot @Fraviiubot Para Poder Manda Sua Sugestão!! ".format(m.from_user.first_name), parse_mode='Markdown')
...
def status(m):
	if m.text == '/status' or m.text == '/status@Amanda_Robot':
